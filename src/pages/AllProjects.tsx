...
const categories = ['All', 'Healthcare', 'Logistics', 'E-Commerce', 'Learning Management', 'Analytics', 'Cybersecurity', 'AI & Automation'];
      ...
      <section className="py-32 max-w-[1440px] mx-auto px-8 lg:px-16">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs font-medium tracking-widest uppercase text-[#6B6B6B]">Filter</span>
          </div>
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className={viewMode === 'grid' ? 'grid gap-px bg-[#D4D3CF] grid-cols-1 md:grid-cols-2 lg:grid-cols-3' : 'space-y-px bg-[#D4D3CF]'}
          >
                ...
                {viewMode === 'grid' ? (
                  <div className="bg-[#EDECE8] group h-full flex flex-col hover:shadow-lg transition-all duration-500">
                    ...
                    <div className="p-6 flex flex-col flex-1">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="text-xs font-medium tracking-widest uppercase" style={{ color: p.accent }}>
                          {p.category}
                        </span>
                      </div>
                      ...
                      <p className="text-[#6B6B6B] text-sm leading-relaxed mb-4 flex-1">{p.desc}</p>
                      <div className="flex flex-wrap gap-1.5 pt-4 border-t border-[#D4D3CF]">
                        {p.tags.slice(0, 2).map((tag) => (
                          <span key={tag} className="px-2 py-1 text-xs border border-[#D4D3CF] text-[#6B6B6B] rounded">
                            {tag}
                          </span>
                        ))}
                        {p.tags.length > 2 && (
                  ...
                  <div className="bg-[#EDECE8] p-6 lg:p-8 group flex gap-8 items-center hover:bg-[#F5F4F0] transition-colors duration-300">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <h3 className="text-2xl font-bold mb-2 tracking-tight" style={{ fontFamily: 'Syne, sans-serif' }}>
                        {p.title}
                      </h3>
                      <p className="text-[#6B6B6B] text-sm leading-relaxed mb-4">{p.desc}</p>
                      <div className="flex flex-wrap gap-2">
                        {p.tags.map((tag) => (
                          <span key={tag} className="px-3 py-1 text-xs border border-[#D4D3CF] text-[#6B6B6B]">
                            {tag}
                          </span>
                        ))}
                      </div>
        ...
        >
          <p className="text-[#6B6B6B] text-sm">
            Showing <span style={{ color: '#0D0D0D', fontWeight: 600 }}>{filteredProjects.length}</span> project{filteredProjects.length !== 1 ? 's' : ''}
          </p>
        </motion.div>
      </section>
